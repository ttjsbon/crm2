    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="详情" prop="desc">
        <template slot-scope="scope">
          ...
          <el-button type="primary" size="mini" @click="showDesc(scope.row.description)">查看</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="是否上架" prop="isOnSale" width="150">
        <template slot-scope="scope">
          <div class="flex">
            <el-tag :type="scope.row.isOnSale ? 'success' : 'error' ">{{scope.row.isOnSale ? '已上架' : '在库中'}}</el-tag>
            <el-button class="statetag" v-if='!scope.row.isOnSale' type="primary" size="mini" @click="setState(scope.row, true)">上架</el-button>
            <el-button class="statetag" v-if='scope.row.isOnSale' type="danger" size="mini" @click="setState(scope.row, false)">下架</el-button>
          </div>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        if (sessionStorage.getItem('listQueryData')) {
          this.listQuery = JSON.parse(sessionStorage.getItem('listQueryData'))
          sessionStorage.removeItem('listQueryData')
        }
      ...
      handleUpdate(row) {
        sessionStorage.setItem('listQueryData', JSON.stringify(this.listQuery))
        this.$router.push({
          path: '/goods/edit',
          query: {
            id: row.id
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['商品ID', '商品编号', '名称', '专柜价格', '当前价格', '是否新品', '是否热品', '是否在售', '首页主图', '宣传图片列表', '商品介绍',
            '详细介绍', '商品图片', '商品单位', '关键字', '类目ID', '品牌商ID'
          ]
          const filterVal = ['id', 'goodsSn', 'name', 'counterPrice', 'retailPrice', 'isNew', 'isHot', 'isOnSale',
            'listPicUrl', 'gallery', 'brief', 'desc', 'picUrl', 'goodsUnit', 'keywords', 'categoryId', 'brandId'
          ]
