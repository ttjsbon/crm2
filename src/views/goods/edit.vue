      ...
      <el-form :rules="rules" ref="goods" :model="goods" label-width="150px">
        ...
        <el-form-item label="宣传画廊">
          <el-upload :action='uploadPath' :limit='5' multiple accept=".jpg,.jpeg,.png,.gif" :file-list="galleryFileList"
            list-type="picture-card" :on-exceed='uploadOverrun' :on-success="handleGalleryUrl" :on-remove="handleRemove">
  ...
  export default {
    ...
    methods: {
      init: function() {
        ...
        detailGoods(goodsId).then(response => {
          ...
          if (this.products.length && this.products[0].productFinances && this.products[0].productFinances.length) {
            this.stagingType = this.products[0].productFinances[0].periodType
            ...
            this.products[0].productFinances.forEach(item => {
              this.leaseTerm.push(item.periods)
      ...
      changetenancyType(val) {
        // this.specifications = this.specifications.filter(item => {
        //   return item.specification !== '分期类型'
        // })
        // var obj = {}
        // obj.specification = '分期类型'
        // if (val === 1) {
        //   obj.value = '按月计算'
        // } else if (val === 2) {
        //   obj.value = '按天计算'
